...
struct CXMLReader::SImplementation{
    ...
    void CharacterDataHandler(const std::string &cdata){
        if (!cdata.empty()) { // Ignore empty character data
            if (DEntityQueue.empty() || DEntityQueue.back().DType != SXMLEntity::EType::CharData) {
                ...
                // Append character data to the existing CharData entity
    ...
    bool ReadEntity(SXMLEntity &entity, bool skipcdata){
        ...
        while(DEntityQueue.empty()){
            ...
            if(DDataSource->Read(DataBuffer, 2560))
                XML_Parse(DXMLParser,DataBuffer.data(), DataBuffer.size(), DataBuffer.size() < 2560);
